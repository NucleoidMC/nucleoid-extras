...
public class QuickArmorStandItem extends Item implements VirtualItem {
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (direction == Direction.DOWN) {
            ...
            Box box = NEEntities.QUICK_ARMOR_STAND_ENTITY_ENTITY_TYPE.getDimensions().getBoxAt(vec3d.getX(), vec3d.getY(), vec3d.getZ());
            if (world.isSpaceEmpty(null, box, (entity) -> true) && world.getOtherEntities(null, box).isEmpty()) {
                if (world instanceof ServerWorld serverWorld) {
                    var armorStandEntity = NEEntities.QUICK_ARMOR_STAND_ENTITY_ENTITY_TYPE.create(serverWorld, itemStack.getTag(), null, context.getPlayer(), blockPos, SpawnReason.SPAWN_EGG, true, true);
